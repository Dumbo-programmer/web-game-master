...
db.collection('credits').doc(localStorage.getItem("ID")).get().then((doc) => {
    localStorage.setItem("temp2", doc.data().username)
})
if(!localStorage.getItem('ptemp2')){
    localStorage.setItem("ptemp2", "Computer2")
}else if(!localStorage.getItem('ptemp3')){
    localStorage.setItem("ptemp3", "Computer3")
}else if(!localStorage.getItem('ptemp1')){
    localStorage.setItem("ptemp1", "Computer1")
}else{
...
db.collection("credits").where('online', "==", true).get().then((querySnapshot) => {
    querySnapshot.forEach((doc) => {
       myArray.push(doc.data().username)
...
db.collection('credits').doc(localStorage.getItem('ID')).get().then((doc) => {
 credito = doc.data().credits;
 localStorage.setItem('credito', credito)
  console.log(doc.data().credits);
  })

var inc = parseInt(localStorage.getItem('credito'))
db.collection('credits').doc(id).set({
  credits: inc+100
}, {merge: true})
}
...
var step_sound = new Audio('step.mp3');
var dead_sound = new Audio('dead.mp3');
var inout_sound = new Audio('inout.mp3');
var dice_sound = new Audio('dice.mp3');
var winner_sound = new Audio('winner.mp3');
...
var names = {
    red_player_name: localStorage.getItem('ptemp1'),
    green_player_name: localStorage.getItem('ptemp2'),
    blue_player_name: localStorage.getItem('temp2'),
    yellow_player_name: localStorage.getItem('ptemp3'),
}

//Concept_warrior_10.png
db.collection('names').doc(localStorage.getItem('ptemp1')).get().then((doc) => {
    document.querySelector('#move-blue1').style.backgroundImage = 'url(../'+doc.data().profilepic+')'
})
///
db.collection('names').doc(localStorage.getItem('ptemp2')).get().then((doc) => {
    document.querySelector('#move-blue2').style.backgroundImage = 'url(../'+doc.data().profilepic+')'
})
//////
db.collection('names').doc(localStorage.getItem('ptemp3')).get().then((doc) => {
    document.querySelector('#move-blue3').style.backgroundImage = 'url(../'+doc.data().profilepic+')'
})
if(localStorage.getItem('profilepic')<10){
document.querySelector('#move-blue4').style.backgroundImage = 'url(../Concept_warrior_0'+localStorage.getItem('profilepic')+'.png'+')' 
}else{
    document.querySelector('#move-blue4').style.backgroundImage = 'url(../Concept_warrior_'+localStorage.getItem('profilepic')+'.png'+')' 
}
...
var bot = {
   ...
   // $("#red_player_name .bot").toggleClass("boton");
    ...
    $("#red_player_name .cun").attr("disabled", bot.red);
        ...
        $("#red_player_name .cun").val(localStorage.getItem('ptemp1'));
    ...
    $("#green_player_name .cun").attr("disabled", bot.green);
        $("#green_player_name .cun").val(localStorage.getItem('ptemp2'));
  //  $("#blue_player_name .bot").toggleClass("botoff");
  //  $("#blue_player_name .bot").toggleClass("boton");
     $("#blue_player_name .cun").attr("disabled");
    $("#blue_player_name .cun").val(localStorage.getItem("temp2"));
       // $("#blue_player_name .cun").attr("disabled");
  bot.yellow = !bot.yellow;
  $("#yellow_player_name .cun").attr("disabled", bot.yellow);
 $("#yellow_player_name .cun").val(localStorage.getItem('ptemp3'));
        ...
        for (const item of run_point) {
            ...
            if (M_Player.path[M_Player.current_position] == 'win') {
                ...
                if (getWinPlayers(M_Player.home) > 3) {
                    ...
                    $(M_Player.home + " .white-box").css("background-image", "url(crown" + rank_count + ".png)");
...
function activateDice() {
    $("#dice").css("opacity", "1");
    dice_value = 0;
    // $("#dice").css("background-image","url('/dice"+dice_value+".png')");
    $("#dice").html("<img src='dice" + dice_value + ".png' class='dice'/>");
